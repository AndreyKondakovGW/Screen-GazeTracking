...
std::vector<FaceInferenceResults> FaceDetector::detect(const cv::Mat& image, const rs2::depth_frame& depth_frame) {
    ...
    for (unsigned long detectionID = 0; detectionID < numTotalDetections; ++detectionID) {
        ...
        tmp.disttoface = depth_frame.get_distance(faceRect.x + (faceRect.width / 2), faceRect.y + (faceRect.height / 2));
        detectionResult.push_back(tmp);
        if (detectionResult[0].faceDetectionConfidence < tmp.faceDetectionConfidence){
            detectionResult[0] = tmp;
        }
    }
