...
void ResultsMarker::mark(cv::Mat& image,
    ...
    //Convert gaze vector to screen point and draw it
    //cv::Point3f gaze_vector = cv::Point3f(-1*faceInferenceResults.gazeVector.x, -1*faceInferenceResults.gazeVector.y, faceInferenceResults.gazeVector.z)
    //cv::Point2f between_eyes = cv::Point2f((faceInferenceResults.leftEyeMidpoint.x + faceInferenceResults.rightEyeMidpoint.x) / 2, (faceInferenceResults.leftEyeMidpoint.y + faceInferenceResults.rightEyeMidpoint.y) / 2);
